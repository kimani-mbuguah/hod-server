...
const Client = require("mongoose").model("User")
...
// User changing their profile password - auth priveledges
exports.updateProfilePassword = function(req, res, next) {
  if (!req.body.user || typeof req.body.user !== "object") {
    return res
      .status(409)
      .json({ success: false, errors: ["'user' param is required"] })
  }
...
exports.activate = function(req, res, next) {
  ...
  User.findOneAndUpdate({ _id: req.body.userId }, user, (err2, data) => {
    if (err2) {
      return res.status(400).json({
        success: false,
        message: "An error occurred while activating user",
...
/**
 * @description These routes should only be accessed by the super admin
 * @todo implement get user account balances
 */

// GET /api/clients
// List clients, paginations options
exports.getAllClients = function(req, res, next) {
  ...
  Client.paginate(filterOptions, pageOptions, (err, result) => {
    if (err) {
      console.log(err)
      return res.status(500).json({
        success: false,
